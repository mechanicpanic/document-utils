...
# Constants
MAX_TOKENS = 500000  # Maximum tokens per batch
        ...
        for para in paragraphs:
            ...
            if para_tokens > max_tokens:
                ...
                for sentence in sentences:
                    ...
                    if sentence_tokens + sentence_tokens > max_tokens and sentence_batch:
                ...
                if sentence_batch:
                    if current_batch and (current_tokens + sentence_tokens <= max_tokens):
                        current_batch += "\n\n" + sentence_batch
...
def translate_content(
    ...
    if len(batches) == 1:
        ...
        logger.info(f"Translating entire text to {target_language} using {model} model...")
        ...
        for i, batch in enumerate(batches):
            ...
            batch_prompt = prompt
            if i > 0:
                batch_prompt += "\n\nThis is part " + str(i+1) + " of a longer text. Continue in the same style and terminology as previous parts."
            
            try:
                response = client.models.generate_content(
                    model=model, contents=batch_prompt + "\n\n" + batch
                )
                batch_time = time.time() - batch_start_time
                logger.info(f"Batch {i+1} completed in {batch_time:.2f} seconds")
                translated_batches.append(response.text)
            except Exception as e:
    ...
    for fmt in formats:
        if fmt == "txt":
            ...
            try:
                ...
                original_paragraphs = [p for p in original_text.split("\n\n") if p.strip()]
                translated_paragraphs = [p for p in translated_text.split("\n\n") if p.strip()]
...
def main():
    ...
    if total_tokens > args.batch_size:
        logger.info(f"Content was processed in batches (max {args.batch_size} tokens per batch)")
